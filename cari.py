...
def login():
	...
	if request.method == 'POST':
		...
		if request.form['Simpan']=="login":
			sql ="select * from user_profile where nama = '" + request.form['username'] +"' and password= '"+request.form['password'] + "'"
			...
			args = (request.form['nama'], request.form['passworddaftar'], request.form['usia'], request.form['jk'], request.form['pendapatan'], request.form['pekerjaan'], request.form['anggaran'])
			mycursor.execute(sql, args)
			db.commit()

			sql ="select * from user_profile where nama = '" + request.form['nama'] +"' and password= '"+request.form['passworddaftar'] + "'"
			mycursor.execute(sql)
			myresult = mycursor.fetchall()
			#print  len(myresult)
			if len(myresult) > 0 :
...
def pilih():
	...
	for i in data:		
		sql="SELECT %s FROM car GROUP BY %s HAVING ( COUNT(%s) > 0 )" %(i,i,i)
		mycursor.execute(sql)
		myresult = mycursor.fetchall()
		kiriman=[]
		for i in myresult:
...
def pilihuser(kode):
	...
	data=["merk","jenis","varian","transmisi","warna"]
	for i in data:		
		sql="SELECT %s FROM car %s GROUP BY %s HAVING ( COUNT(%s) > 0 )" %(i,kode,i,i)
		mycursor.execute(sql)
		myresult = mycursor.fetchall()
		kiriman=[]
		for i in myresult:
...
def pilihratinguser(kode):
	...
	data=["merk","jenis","varian","transmisi","warna"]
	for i in data:		
		sql="SELECT a.%s FROM car %s GROUP BY a.%s HAVING ( COUNT(a.%s) > 0 )" %(i,kode,i,i)
		mycursor.execute(sql)
		myresult = mycursor.fetchall()
		kiriman=[]
		for i in myresult:
...
def pilihcari(kode):
	...
	data=["merk","jenis","varian","transmisi","warna"]
	for i in data:		
		sql="SELECT %s FROM car where idmobil in (%s) GROUP BY %s HAVING ( COUNT(%s) > 0 )" %(i,kode,i,i)
		mycursor.execute(sql)
		myresult = mycursor.fetchall()
		kiriman=[]
		for i in myresult:
...
def menu():
	if g.user:
		...
		db = conn()
		mycursor =db.cursor()
		sql="SELECT a.idmobil FROM car as a inner join rating as b on a.idmobil = b.car_id where b.user_id = %s" %(session['id'])
		mycursor.execute(sql)
		myresult = mycursor.fetchall()
		if len(myresult) < 3:
...
def datauser():
	if g.user:		
		...
		if request.method == 'POST':	
			...
			mycursor =db.cursor()
			sql ="INSERT into user_profile(nama, password, usia, jk, pendapatan, pekerjaan, anggaran) values (%s, %s, %s, %s, %s, %s, %s)" 
			args = (request.form['nama'], request.form['password'], request.form['usia'], request.form['jk'], request.form['pendapatan'], request.form['pekerjaan'], request.form['anggaran'])
			mycursor.execute(sql, args)
			db.commit()
	...
	if g.user:
		...
		if request.method == 'POST':
			if request.form['Simpan'] == "Simpan":
				...
				args =(request.form['usia'], request.form['jk'], request.form['pendapatan'], request.form['pekerjaan'], request.form['anggaran'], session['id']) 
				mycursor.execute(sql, args)
...
def tableuser():
	if g.user:
		...
		if request.method == 'POST':
			if request.form['Simpan'] == "Simpan":
				sql = "UPDATE `user_profile` SET usia = %s , jk = %s , pendapatan = %s , pekerjaan = %s , anggaran = %s WHERE id = %s"
				args =(request.form['usia'], request.form['jk'], request.form['pendapatan'], request.form['pekerjaan'], request.form['anggaran'], request.form['kodeuser']) 
				mycursor.execute(sql, args)
			elif request.form['Simpan'] == 'SimpanPass':
				###print "masuk ke sini"
				sql = "UPDATE `user_profile` SET password = %s WHERE id = %s"
				...
				sql = "DELETE FROM user_profile WHERE id = %s" %(request.form['kodeuser'])
				mycursor.execute(sql)
			db.commit()
...
def datamobil():
	if g.user:
		...
		if request.method == 'POST':
			...
			args = (request.form['merk'], request.form['jenis'], request.form['varian'], request.form['cc'], request.form['transmisi'], request.form['harga'], request.form['warna'], request.form['keterangan'], request.form['kelebihan'], request.form['kekurangan'], request.form['image'], request.form['seater'], request.form['aftersale'], gas, dis)
			mycursor.execute(sql, args)
			db.commit()
			###print mycursor.rowcount, "record inserted."
	...
	if g.user:
		...
		if request.method == 'POST':
			if request.form['Simpan'] == "Simpan":
				...
				args = (request.form['merk'], request.form['jenis'], request.form['varian'], request.form['cc'], request.form['transmisi'], request.form['harga'], request.form['warna'], request.form['keterangan'], request.form['kelebihan'], request.form['kekurangan'], request.form['image'], request.form['seater'], request.form['aftersale'], gas, dis, request.form['kodemob'])
				mycursor.execute(sql, args)
			else:
				sql = "DELETE FROM car WHERE idmobil = %s" %(request.form['kodemob'])
				mycursor.execute(sql)
			db.commit()
		sql="select * from car"
		mycursor.execute(sql)
		...
		for i in myresult:
			...
			try:
				...
				print i
				print "=================="
			post.append([i[0], str(i[1]).capitalize(), str(i[2]).capitalize(), str(i[3]).capitalize(), i[4], str(i[5]).capitalize(), i[6], str(i[7]).capitalize(), i[8], i[9], i[10],i[11], filename,i[13],i[14],i[15],i[16]])
		###print post
...
def mobiluser():
	if g.user:
		...
		db = conn()
		mycursor =db.cursor()
		sql="select * from car where harga > (select b.harga1 from user_profile as a inner join hargamobil as b on a.anggaran=b.Ket where a.nama = '%s') and harga < (select b.harga2 from user_profile as a inner join hargamobil as b on a.anggaran=b.Ket where a.nama = '%s')" %(session['user'],session['user'])
		mycursor.execute(sql)
		myresult = mycursor.fetchall()
		post=[]
		z=0
...
def listallmobiluser():
	if g.user:
		...
		if request.method == 'POST':		
			...
			res = listallmobilusercari(merk,jenis,varian,cc,transmisi,harga,warna,seater,aftersale,gasoline)
			myresult=res[0][0]
			s=""
			for t in res[1][0]:
			    s+=str(t[0])+","
			...
			mycursor.execute(sql)
			myresult = mycursor.fetchall()
			sqlpilihuser = " as a inner join rating as b on a.idmobil=b.car_id inner join user_profile as c on b.user_id=c.id where harga > (select b.harga1 from user_profile as a inner join hargamobil as b on a.anggaran=b.Ket where a.nama = '%s') and harga < (select b.harga2 from user_profile as a inner join hargamobil as b on a.anggaran=b.Ket where a.nama = '%s')  and c.nama = '%s' " %(session['user'],session['user'],session['user'])
			kirim=pilih()
			post=[]
			post2=[]
			z=0
...
def PilihMobilRegis():
	if g.user:
		if request.method == 'POST':
			db = conn()
			mycursor =db.cursor()
			sql="SELECT * from rating where car_id = %s and user_id=%s" %(session['kodemob'],session['id'])
			mycursor.execute(sql)
			myresult1 = mycursor.fetchall()
			if len(myresult1)==0:
				sql = "insert into rating(user_id, car_id, ratings) values (%s, %s, %s)"
				args = (session['id'],session['kodemob'],request.form['rating'])
				mycursor.execute(sql, args)
			else:
				sql = "UPDATE rating set ratings = %s where car_id= %s and user_id=%s" %(request.form['rating'],session['kodemob'],session['id'])
				mycursor.execute(sql)
			db.commit()
			sql="SELECT avg(ratings) from rating where car_id = %s" %(session['kodemob'])
			mycursor.execute(sql)
			myresult1 = mycursor.fetchall()
			if len(myresult1) > 0:
...
def ModalPilihMobilRegis(kode):
	...
	sql="SELECT ratings from rating where car_id = %s and user_id=%s" %(kode,session['id'])
	mycursor.execute(sql)
	myresult1 = mycursor.fetchall()
	rat=0
	if len(myresult1) > 0:
	...
	for i in myresult:
		...
		pilImg.save(filename)
		post.append([i[0], str(i[1]).capitalize(), str(i[2]).capitalize(), str(i[3]).capitalize(), i[4], str(i[5]).capitalize(), i[6], str(i[7]).capitalize(), i[8], i[9], i[10],i[11], filename,i[13],i[14],i[15],i[16]])
	return render_template("ModalPilihMobilRegis.html",post=post,rat=rat,kode=kode)

#@application.route('/listallmobilusercari/<merk>/<jenis>/<varian>/<cc>/<transmisi>/<harga>/<warna>', methods=['GET', 'POST'])
def listallmobilusercari2(merk,jenis,varian,cc,transmisi,harga,warna,seater,aftersale,gasoline):
	myresult=[]
	db = conn()
	mycursor =db.cursor()
	###print harga
	if harga == "%":
		###print "kadieu"
		sql="select * from car where merk like '%s' and jenis like '%s' and varian like '%s' and cc like '%s' and transmisi like '%s' and harga like '%s' and warna like '%s' and seater like '%s' and aftersale like '%s' and gasoline like '%s'" %(merk,jenis,varian,cc,transmisi,harga,warna,seater,aftersale,gasoline)
	else:
		###print "kaditu"
		sql="select * from car where merk like '%s' and jenis like '%s' and varian like '%s' and cc like '%s' and transmisi like '%s'  and warna like '%s' and harga > (select harga1 from  hargamobil where Ket = '%s') and harga < (select harga2 from  hargamobil where Ket = '%s') and seater like '%s' and aftersale like '%s' and gasoline like '%s'" %(merk,jenis,varian,cc,transmisi,warna,harga,harga,seater,aftersale,gasoline)
	mycursor.execute(sql)
	myresult = mycursor.fetchall()
	return myresult

def listallmobilusercari(merk,jenis,varian,cc,transmisi,harga,warna,seater,aftersale,gasoline):
	db = conn()
	mycursor =db.cursor()
	if merk == "%":
		merk=""
	...
	ab=("","",merk,jenis,varian,cc,transmisi,warna,harga,seater,aftersale,gasoline)
	...
	for h in myresult:
		...
		for k in ab:                   
		    if k!="":
		        ...
		        train=[]
		        cossim=0
		        if (kolom==2) or (kolom==3) or (kolom==4) or (kolom==6) or (kolom==7):
		            ...
		            for asci in range(len(test)):
		                try:
		                    atas=float(atas)+(float(test[asci])*float(train[asci]))
		                except:
		                    atas=atas
		            ...
		            #print ""
		            d.append(1-cossim)
		        if (kolom==5) or (kolom==8) or (kolom==9) or (kolom==10) or (kolom==11):
		            b=float(k)
		            c=float(h[a])
	...
	for o in m:
	    ...
	    a=a+1
	duaakhir = sorted(sorted(duadata.items(), key=lambda t: t[0]), key=lambda t: t[1])
	tigaakhir = sorted(sorted(tigadata.keys(), key=lambda t: (tigadata[t]['rat']), reverse=True), key=lambda t: float(tigadata[t]['euc']))
	#print "Jarak Euclidean setelah disorting = "
	#print str(duaakhir)
	p5=(((duaakhir[len(duaakhir)-1][1])-(duaakhir[0][1]))*0.05)+(duaakhir[0][1])
	p10=(((duaakhir[len(duaakhir)-1][1])-(duaakhir[0][1]))*0.1)+(duaakhir[0][1])
	p20=(((duaakhir[len(duaakhir)-1][1])-(duaakhir[0][1]))*0.2)+(duaakhir[0][1])
	p40=(((duaakhir[len(duaakhir)-1][1])-(duaakhir[0][1]))*0.4)+(duaakhir[0][1])
	p80=(((duaakhir[len(duaakhir)-1][1])-(duaakhir[0][1]))*0.8)+(duaakhir[0][1])
	#print ""
		...
		if ka==10:
			for lokasi in duaakhir:
				if isi==lokasi[0]:
					...
					akurasi10=(1-((lokasi[1]-duaakhir[0][1])/(duaakhir[len(duaakhir)-1][1]-duaakhir[0][1])))*100
			#print ""
		...
		if ka==50:
			for lokasi in duaakhir:
				if isi==lokasi[0]:
					...
					akurasi50=(1-((lokasi[1]-duaakhir[0][1])/(duaakhir[len(duaakhir)-1][1]-duaakhir[0][1])))*100
			#print ""
		...
		if ka==100:
			for lokasi in duaakhir:
				if isi==lokasi[0]:
					...
					akurasi100=(1-((lokasi[1]-duaakhir[0][1])/(duaakhir[len(duaakhir)-1][1]-duaakhir[0][1])))*100
			#print ""
		...
		if ka==500:
			for lokasi in duaakhir:
				if isi==lokasi[0]:
					...
					akurasi500=(1-((lokasi[1]-duaakhir[0][1])/(duaakhir[len(duaakhir)-1][1]-duaakhir[0][1])))*100
			#print ""
		...
		if ka==1000:
			for lokasi in duaakhir:
				if isi==lokasi[0]:
					...
					akurasi1000=(1-((lokasi[1]-duaakhir[0][1])/(duaakhir[len(duaakhir)-1][1]-duaakhir[0][1])))*100
			#print ""
...
def listallmobilusercarirelated(merk,jenis,varian,cc,transmisi,harga,warna,seater,aftersale,gasoline):
	db = conn()
	mycursor =db.cursor()
	if merk == "%":
		merk=""
	...
	duaakhir = sorted(sorted(duadata.items(), key=lambda t: t[0]), key=lambda t: t[1])
	tigaakhir = sorted(sorted(tigadata.keys(), key=lambda t: (tigadata[t]['rat']), reverse=True), key=lambda t: float(tigadata[t]['euc']))
	p=(((duaakhir[len(duaakhir)-1][1])-(duaakhir[0][1]))*0.2)+(duaakhir[0][1])
	r=[]
	for isi in tigaakhir:
		for q in duaakhir:
			if q[0]==isi:
	...
	if g.user:
		...
		if request.method == 'POST':		
			...
			for i in myresult:
				...
				filename = "static/image/"+str(i[0])+".png"
				pilImg.save(filename)
				post.append([i[0], str(i[1]).capitalize(), str(i[2]).capitalize(), str(i[3]).capitalize(), i[4], str(i[5]).capitalize(), i[6], str(i[7]).capitalize(), i[8], i[9], i[10],i[11], filename])
				if z<=10:
					post2.append([i[0], str(i[1]).capitalize(), str(i[2]).capitalize(), str(i[3]).capitalize(), i[4], str(i[5]).capitalize(), i[6], str(i[7]).capitalize(), i[8], i[9], i[10],i[11], filename,i[13],i[14],i[15],i[16]])
			return render_template("listallmobiluser.html",
									post=post,
			...
			db = conn()
			mycursor =db.cursor()
			sql="select * from car where harga < (select b.harga2 from user_profile as a inner join hargamobil as b on a.anggaran=b.Ket where a.nama = '%s')" %(session['user'])
			mycursor.execute(sql)
			myresult = mycursor.fetchall()
			sqlpilihuser = " where harga < (select b.harga2 from user_profile as a inner join hargamobil as b on a.anggaran=b.Ket where a.nama = '%s')" %(session['user'])
...
def listratinglmobiluser():
	if g.user:
		...
		if request.method == 'POST':		
			...
			db = conn()
			mycursor =db.cursor()
			sql="SELECT * from car as a inner join rating as b on a.idmobil=b.car_id inner join user_profile as c on b.user_id=c.id where c.id = '%s'" %(session['id'])
			mycursor.execute(sql)
			myresult = mycursor.fetchall()
			sqlpilihuser = " as a inner join rating as b on a.idmobil=b.car_id inner join user_profile as c on b.user_id=c.id where  c.id = '%s' " %(session['id'])
...
def listallmobilusersinggle(kode):
	if g.user:
		...
		for i in myresult:
			...
			filename = "static/image/"+str(i[0])+".png"
			pilImg.save(filename)
			sql="select ratings from rating where car_id = %s and user_id=%s" %(i[0],session['id'])
			mycursor.execute(sql)
			myresult1 = mycursor.fetchall()
			rat=0
			if len(myresult1) > 0:
		...
		res2 = listallmobilusercarirelated(merk,jenis,varian,cc,transmisi,harga,warna,seater,aftersale,gasoline)
		myresult2=res2[0][0]
		relatedpost=[]
		for i in myresult2:
			z=z+1
			...
			pilImg.save(filename)
			if z<=11:
				relatedpost.append([i[0], str(i[1]).capitalize(), str(i[2]).capitalize(), str(i[3]).capitalize(), i[4], str(i[5]).capitalize(), i[6], str(i[7]).capitalize(), i[8], i[9], i[10],i[11], filename,i[13],i[14],i[15],i[16]])

		return render_template("listallmobilusersinggle.html",post=post,rat=rat,relatedpost=relatedpost)
	return redirect(url_for('menu'))
...
def rating(kode):
	if g.user:
		...
		mycursor =db.cursor()		
		if request.method == 'POST':
			sql="SELECT * from rating where car_id = %s and user_id=%s" %(kode,session['id'])
			mycursor.execute(sql)
			myresult1 = mycursor.fetchall()
			if len(myresult1)==0:
				sql = "insert into rating(user_id, car_id, ratings) values (%s, %s, %s)"
				...
				sql = "UPDATE rating set ratings = %s where car_id= %s and user_id=%s" %(request.form['rating'],kode,session['id'])
				mycursor.execute(sql)
			db.commit()
		...
		for i in myresult2:
			...
			if z<=11:
				relatedpost.append([i[0], str(i[1]).capitalize(), str(i[2]).capitalize(), str(i[3]).capitalize(), i[4], str(i[5]).capitalize(), i[6], str(i[7]).capitalize(), i[8], i[9], i[10],i[11], filename,i[13],i[14],i[15],i[16]])
		return render_template("rating.html",post=post,rat=rat,relatedpost=relatedpost)
	return redirect(url_for('menu'))
...
def simattmetode2(kode):
    usia={'19 - 30 tahun':1,'31 - 40 tahun':2,'41 - 50 tahun':3,'51 - 60 tahun':4,'lebih dari 60 tahun':5}
    jk={'Laki-laki':2,'Perempuan':1}
    pendapatan={'Kurang dari Rp.8 juta':1,'Antara Rp.8 juta hingga Rp.15 juta':2,'Lebih dari Rp.15 juta':3}
    pekerjaan={'Pelajar':1,'Wiraswasta':2,'Pegawai Negeri Sipil':4,'Pegawai Swasta':3,'Professional':5}
    anggaran={'Kurang dari Rp.200 juta':1,'Rp.200 juta - Rp.400 juta':2,'Rp.400 juta - Rp.600 juta (tepat Rp.400 juta tidak termasuk)':3,'Rp.600 juta - Rp.800 juta (tepat Rp.600 juta pas tidak termasuk)':4,'Lebih dari Rp.800 juta':5}
    ...
    myresult = mycursor.fetchall()
    for a in myresult:
        data.append([usia[a[3]],jk[a[4]],pendapatan[a[5]],pekerjaan[a[6]],anggaran[a[7]]])
    ...
    if len(myresult3) > 0:
    	...
    	sql2="SELECT * from (SELECT a.id as id, a.nama as nama, a.password as password, a.usia as usia, a.jk as jk, a.pendapatan as pendapatan, a.pekerjaan as pekerjaan, a.anggaran as anggaran, b.ratings as ratings from user_profile as a left join rating as b on a.id=b.user_id where `usia` <> '0' group by a.id) as c where c.ratings <> 'null' and c.usia <> '0' and c.id <> '0' and c.id <> %s" %(kode)
    	##print "0000002"
    
    mycursor.execute(sql2)
    myresult2 = mycursor.fetchall()
    for a in myresult2:
        data2.append([usia[a[3]],jk[a[4]],pendapatan[a[5]],pekerjaan[a[6]],anggaran[a[7]]])

    for a in range(0,len(data2)):
        atas = ((data[0][0]*data2[a][0])+(data[0][1]*data2[a][1])+(data[0][2]*data2[a][2])+(data[0][3]*data2[a][3])+(data[0][4]*data2[a][4]))
        bawah1 = (((data[0][0]**2)+(data[0][1]**2)+(data[0][2]**2)+(data[0][3]**2)+(data[0][4]**2))**0.5)
        bawah2 = (((data2[a][0]**2)+(data2[a][1]**2)+(data2[a][2]**2)+(data2[a][3]**2)+(data2[a][4]**2))**0.5)
...
def ratingattmetode2(kode):  
    ...
    sql="SELECT a.user_id,c.nama,a.car_id,a.ratings from rating as a inner join car as b on a.car_id = b.idmobil inner join user_profile as c on a.user_id=c.id and c.usia <> '0' and c.id <> '0' where a.user_id =%s" %(kode) 
    mycursor.execute(sql)
    myresult = mycursor.fetchall()
    ...
    for a in user: #mengambil satu persatu user dan dan semua ratingnya
        ...
        for b in a: #mengambil satu user dan satu persatu ratingnya
            ...
            for c in myresult: #mengambil baris pada user login               
                if b[2]==c[2]: #id_car pada array b(user yang diambil) sama dengan id_car pada array c(user login)
                    atas = atas + (b[3]*c[3])              
                bawah2 = bawah2 + (c[3]**2)
            bawah1 = bawah1 + (b[3]**2)
            k=k+1
...
def simuser(kode):
    ...
    for a in range(0,len(simatt)):
    	if len(simrating) == 0:
    		break
        total= float((float(simatt[a][2])*float(0.4))+(float(simrating[a][1])*float(0.6)))
        data.append((simatt[a][0],simatt[a][1],simatt[a][2],simrating[a][1],total))
    args = str(data)[1:][:len(str(data))-2]
    sql ="INSERT into similarity(user_id,user_id2,sim_att,sim_rating,sim_tot) values " + (args)
    mycursor.execute(sql, args)
    db.commit()
    simpred(kode)
...
def simpred(kode):
    ...
    sql="SELECT * from (SELECT b.user_id as user_id, b.car_id as car_id, b.ratings as ratings from  user_profile as a \
         ...
         where d.user_id <> %s" %(kode) #user_id | car_id (berurutan) | ratings
    mycursor.execute(sql)
    myresult2 = mycursor.fetchall()
    ...
    for a in myresult2:
        n+=1
        sql="SELECT user_id, car_id, ratings from rating where car_id = %s" %(a[1]) #mengambil car_id pada setiap baris myresult2
        mycursor.execute(sql)
        myresult3 = mycursor.fetchall()
        atas1 = 0
        bawah1 = 0
        for b in myresult3:
            if b[0] != 0: #mengambil user_id dari myresult3
                sql="SELECT sim_tot from similarity where user_id = %s and user_id2 = %s" %(kode,b[0]) 
                mycursor.execute(sql)
                ...
                atas1=float(float(atas1) + float((float(b[2]-Ru1)) * float(myresult4[0][0])))
    ...
    if args != "":
	    sql ="INSERT into prediction(user_id,car_id,pre_rating) values " + (args)
	    mycursor.execute(sql)
	    db.commit()
...
def ratingattmetode3(kode):
    db = conn()
    mycursor =db.cursor()
    sql="SELECT user_id, car_id, ratings from rating where user_id = %s order by user_id asc" %(kode)
    mycursor.execute(sql)
    myresult = mycursor.fetchall()
    ...
    for a in myresult:
        for b in myresult4:     
            ...
            for c in myresult2:
                ...
                atas = float(atas + ((float(myresult7[0][2]) - float(c[3])) * (float(myresult8[0][2]) - float(c[3]))))
...
def simattmetode3(kode):
    ...
    mobil={"toyota" : 1, "Daihatsu" : 2, "mitsubishi" : 3, "Honda" : 4, "suzuki" : 5, "isuzu" : 6, "Datsun" : 7, "nissan" : 8, "wuling" : 9, "mazda" : 10, "kia" : 11, "BMW" : 12, "Chevrolet" : 13, "ford" : 14, "jeep" : 15, "land rover" : 16, "lexus" :  17, "mercedes benz" : 18, "mini cooper" : 19, "hyundai" : 20}
    seat={2:1, 4:2, 5:3, 7:4, 8:5, 9:6}
    fuel={1:1,0:2}

    db = conn()
    mycursor =db.cursor()
    sql="SELECT a.user_id, b.cc, b.warna, b.harga, b.transmisi, b.idmobil, b.aftersale, b.seater, b.gasoline, b.merk from rating as a inner join car as b on b.rating > 0 and a.car_id=b.idmobil where a.user_id = %s" %(kode)
    mycursor.execute(sql)
    myresult = mycursor.fetchall()
    ...
    for b in range(0,len(myresult)):
        ...
        for a in myresult2:
            ...
            data1 = (dat_mobil3*(16.5/100))+(dat_harga3*(15.7/100))+ (dat_trans3*(15.7/100))+(dat_warna3*(15.3/100))+(dat_cc3*(15.1/100))+(dat_seat3*(13.4/100))+(dat_fuel3*(7.5/100))+(dat_hargapurna3*(0.8/100))
...
def simmobil(kode):
    ...
    for a in range(0,len(simatt)):
        ...
        data.append((simatt[a][0],simatt[a][1],simatt[a][2],simrating[a][1],total))
    args = str(data)[1:][:len(str(data))-2]
    sql ="INSERT into similarity_car(idmobil,idmobil2,sim_att,sim_rating,sim_tot) values " + (args)    
    mycursor.execute(sql, args)
    db.commit()
    simpred_car(kode)
...
def simpred_car(kode):  
    ...
    mycursor.execute(sql)
    myresult = mycursor.fetchall()
    sql="SELECT user_id, car_id, ratings from rating where user_id = %s" %(kode) 
    mycursor.execute(sql)
    myresult2 = mycursor.fetchall()
    ...
    for a in myresult:
        ...
        #print str(urut+1)
        for b in myresult2:
            sql="SELECT sim_tot from similarity_car where idmobil = %s and idmobil2 = %s" %(b[1],a[1])
            mycursor.execute(sql)
            myresult3 = mycursor.fetchall()
            if len(myresult3) == 0:
                continue
    ...
    args = str(data)[1:][:len(str(data))-2]
    sql ="INSERT into prediction_car(user_id,car_id,pre_rating) values " + (args)    
    mycursor.execute(sql, args)
    db.commit()
	...
	if g.user:
		...
		if request.method == 'POST':		
			...
			for i in myresult:
				...
				filename = "static/image/"+str(i[0])+".png"
				pilImg.save(filename)
				predcar.append([i[0], str(i[1]).capitalize(), str(i[2]).capitalize(), str(i[3]).capitalize(), i[4], str(i[5]).capitalize(), i[6], str(i[7]).capitalize(), i[8], i[9], i[10],i[11], filename,i[13],i[14],i[15],i[16]])
			...
			for i in myresult:
				...
				filename = "static/image/"+str(i[0])+".png"
				pilImg.save(filename)
				preduser.append([i[0], str(i[1]).capitalize(), str(i[2]).capitalize(), str(i[3]).capitalize(), i[4], str(i[5]).capitalize(), i[6], str(i[7]).capitalize(), i[8], i[9], i[10],i[11], filename,i[13],i[14],i[15],i[16]])
			...
			for i in myresult:
				...
				filename = "static/image/"+str(i[0])+".png"
				pilImg.save(filename)
				ratcar.append([i[0], str(i[1]).capitalize(), str(i[2]).capitalize(), str(i[3]).capitalize(), i[4], str(i[5]).capitalize(), i[6], str(i[7]).capitalize(), i[8], i[9], i[10],i[11], filename,i[13],i[14],i[15],i[16]])
			kirim2=pilih()
			return render_template("menu.html",predcar=predcar,preduser=preduser,ratcar=ratcar,kirim2=kirim2)
	return redirect(url_for('menu'))
...
def background_process():
	...
	for i in data:		
		...
		sql="SELECT %s FROM car where upper(merk) like upper('%s') and upper(jenis) like upper('%s') and upper(varian) like upper('%s') and upper(transmisi) like upper('%s') and  upper(warna) like upper('%s') GROUP BY %s HAVING ( COUNT(%s) > 0 )" %(i,merk,jenis,varian,transmisi,warna,i,i)
		mycursor.execute(sql)
		myresult = mycursor.fetchall()
